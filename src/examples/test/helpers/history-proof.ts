...
export function getHistoryProofDirPath(provedBlocksCount: bigint, isDefaultAddr: boolean = true): string {
  const fileNameSuffix = isDefaultAddr ? `_d` : `_c`;

  return path.join(__dirname, `../data/history_proof`, `${provedBlocksCount.toString()}${fileNameSuffix}`);
}
